...
public final class FlagDescriptors {
    ...
    public static final FlagDescriptor[] L2_OS_CONTROL_FLAGS = {
            new FlagDescriptorI("FG_CTRL_SEL_GP", 1 << 0,
                                "Grid point selected according to land sea mask. Least significant bit"),
            new FlagDescriptorI("FG_CTRL_RANGE", 1 << 1, "Retrieved values outside range. Least significant Bit"),
            new FlagDescriptorI("FG_CTRL_SIGMA", 1 << 2, "High retrieval sigma"),
            new FlagDescriptorI("FG_CTRL_CHI2", 1 << 3, "Poor fit quality"),
            new FlagDescriptorI("FG_CTRL_CHI2_P", 1 << 4, "Poor fit quality"),
            new FlagDescriptorI("FG_CTRL_QUALITY_SSS", 1 << 5,
                                "At least one critical flag was raised during SSS1 retrieval"),
            new FlagDescriptorI("FG_CTRL_SUNGLINT", 1 << 6,
                                "Grid point with number of measurements flagged for sunglint above threshold"),
            new FlagDescriptorI("FG_CTRL_MOONGLINT", 1 << 7,
                                "Grid point with number of measurements flagged for moonglint above threshold"),
            new FlagDescriptorI("FG_CTRL_GAL_NOISE", 1 << 8,
                                "Grid point with number of measurements flagged for galactic noise above threshold"),
            new FlagDescriptorI("FG_CTRL_GAL_NOISE_POL", 1 << 9,
                                "Grid point with number of measurements flagged for polarised galactic noise above threshold"),
            new FlagDescriptorI("FG_CTRL_REACH_MAXITER", 1 << 10,
                                "Maximum number of iteration reached before convergence"),
            new FlagDescriptorI("FG_CTRL_NUM_MEAS_MIN", 1 << 11, "Not processed due to too few valid measurements"),
            new FlagDescriptorI("FG_CTRL_NUM_MEAS_LOW", 1 << 12,
                                "Number of valid measurements used for retrieval is less than Tg_num_meas_valid"),
            new FlagDescriptorI("FG_CTRL_MANY_OUTLIERS", 1 << 13,
                                "If number of outliers Dg_num_outliers > Tg_num_outliers_max"),
            new FlagDescriptorI("FG_CTRL_MARQ", 1 << 14,
                                "Iterative loop ends because Marquardt increment is greather than lambdaMax"),
            new FlagDescriptorI("FG_CTRL_ROUGHNESS", 1 << 15, "Roughness correction applied"),
            new FlagDescriptorI("FG_CTRL_FOAM", 1 << 16,
                                "Wind speed is less than Tg_WS_foam and foam contribution and foam fraction are set to zero"),
            new FlagDescriptorI("FG_CTRL_ECMWF", 1 << 17,
                                "Flag set to false if one or more ECMWF data is missing for the different models. Most significant Bit"),
            new FlagDescriptorI("FG_CTRL_VALID", 1 << 18,
                                "Flags raised if grid points pass grid point measurement discrimination tests"),
            new FlagDescriptorI("FG_CTRL_NO_SURFACE", 1 << 19,
                                "Flags raised if the 42.5° angle is not included in the dwell line for grid points"),
            new FlagDescriptorI("FG_CTRL_RANGE_ACARD", 1 << 20, "Flags raised if retrieved Acard is outside range"),
            new FlagDescriptorI("FG_CTRL_SIGMA_ACARD", 1 << 21, "Flags raised if retrieved Acard sigma is too high"),
            new FlagDescriptorI("FG_CTRL_QUALITY_ACARD", 1 << 22,
    ...
    public static final FlagDescriptor[] L2_OS_SCIENCE_FLAGS = {
            new FlagDescriptorI("FG_SC_LAND_SEA_COAST1", 1 << 0,
                                "Distance from coast to gridpoint is less than threshold Max1 in file AUX_DISTAN"),
            new FlagDescriptorI("FG_SC_LAND_SEA_COAST2", 1 << 1,
                                "Distance from coast to gridpoint is less than threshold Max2 in file AUX_DISTAN"),
            new FlagDescriptorI("FG_SC_TEC_GRADIENT", 1 << 2, "High TEC gradient along dwell for a grid point"),
            new FlagDescriptorI("FG_SC_IN_CLIM_ICE", 1 << 3,
                                "Gridpoint with maximum extend of sea ice accordy to monthly climatology"),
            new FlagDescriptorI("FG_SC_ICE", 1 << 4,
                                "Ice concentration at gridpoint is above threshold Tg_ice_concentration"),
            new FlagDescriptorI("FG_SC_SUSPECT_ICE", 1 << 5, "Suspect ice on gridpoint"),
            new FlagDescriptorI("FG_SC_RAIN", 1 << 6,
                                "Heavy rain suspected on gridpoint. Rain rate is above threshold Tg_max_rainfall"),
            new FlagDescriptorI("FG_SC_HIGH_WIND", 1 << 7, "High wind"),
            new FlagDescriptorI("FG_SC_LOW_WIND", 1 << 8, "Low wind"),
            new FlagDescriptorI("FG_SC_HIGHT_SST", 1 << 9, "High SST"),
            new FlagDescriptorI("FG_SC_LOW_SST", 1 << 10, "Low SST"),
            new FlagDescriptorI("FG_SC_HIGH_SSS", 1 << 11, "High SSS"),
            new FlagDescriptorI("FG_SC_LOW_SSS", 1 << 12, "Low SSS"),
            new FlagDescriptorI("FG_SC_SEA_STATE_1", 1 << 13, "Sea state class 1"),
            new FlagDescriptorI("FG_SC_SEA_STATE_2", 1 << 14, "Sea state class 2"),
            new FlagDescriptorI("FG_SC_SEA_STATE_3", 1 << 15, "Sea state class 3"),
            new FlagDescriptorI("FG_SC_SEA_STATE_4", 1 << 16, "Sea state class 4"),
            new FlagDescriptorI("FG_SC_SEA_STATE_5", 1 << 17, "Sea state class 5"),
            new FlagDescriptorI("FG_SC_SEA_STATE_6", 1 << 18, "Sea state class 6"),
            new FlagDescriptorI("FG_SC_SST_FRONT", 1 << 19, "Not implemented yet"),
            new FlagDescriptorI("FG_SC_SSS_FRONT", 1 << 20, "Not implemented yet"),
            new FlagDescriptorI("FG_SC_ICE_ACARD", 1 << 21, "Ice flag from cardioid"),
    };
